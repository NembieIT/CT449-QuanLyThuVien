        ...
        <div v-if="isLoaded"
            ...
            <a-form :model="formState" name="basic" :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }"
                ...
                <h1 class="text-2xl font-bold font-text1">
                    {{ userEdit ? 'CHỈNH SỬA NHÂN VIÊN' : 'THÊM NHÂN VIÊN MỚI' }}
                </h1>
                ...
                <div class="w-full">
                    <div class="flex items-center justify-center gap-5">
                        <a-form-item label="username" name="usernameNV" class="w-1/3"
                            :rules="!userEdit?[{ required: true, message: 'Hãy điền username!' }]:[]">
                            <a-input v-model:value="formState.usernameNV" :placeholder="userEdit?.usernameNV" />
                        </a-form-item>
                        <a-form-item label="Tên" name="nameNV"
                            :rules="!userEdit?[{ required: true, message: 'Hãy điền tên!' }]:[]" class="w-1/3">
                            <a-input v-model:value="formState.nameNV" :placeholder="userEdit?.nameNV" />
                        </a-form-item>
                    </div>
                    ...
                    <div class="flex items-center justify-center gap-5">
                        <a-form-item label="Địa chỉ" name="addressNV"
                            ...
                            <a-input v-model:value="formState.addressNV" :placeholder="userEdit?.addressNV" />
                        </a-form-item>
                        <a-form-item label="SĐT" name="phoneNV"
                            :rules="!userEdit?[{ required: true, message: 'Hãy điền sđt!' }]:[]" class="w-1/3">
                            <a-input v-model:value="formState.phoneNV" :placeholder="userEdit?.phoneNV" />
                        </a-form-item>
                    </div>
...
onMounted(async () => {
    ...
    userEdit.value = (await NVControllerApi.getID(id.value))?.NV;
    isLoaded.value = true;
})
</script>