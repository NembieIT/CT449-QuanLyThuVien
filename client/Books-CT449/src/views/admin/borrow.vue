        ...
        <div class="w-[95%] lg:w-[80vw] h-full flex flex-col gap-5 rounded-2xl p-5 bg-white">
            ...
            <div v-if="!loading" class="flex flex-col gap-[5%] h-[55%] overflow-hidden">
                ...
                <BorrowItem v-if="page=='pending' || page==='all'" v-for="(item, index) in visibleTask" :key="index"
                    :prop1="item.name" :prop2="item.email" :prop3="item.sdt" :status="page=='user'? '': item.status"
                    :page="page">
                </BorrowItem>
                ...
                <BorrowItem v-if="page==='nhanvien'" v-for="(item, index) in visibleTask" :key="index" :id="item._id"
                    :prop1="item.nameNV" :prop2="item.addressNV" :prop3="item.phoneNV" :page="page"
                    @details="handleDetails" @delete="handleDelete">
                </BorrowItem>
...
<script setup>
    ...
    function FilterDataAll() {
        dataPending = dataAll.filter(item => item.status === 'pending');
        dataLate = dataAll.filter(item => item.status.includes('late'));
        dataDone = dataAll.filter(item => item.status === 'done');
        cntDone = dataDone.length;
        cntLate = dataLate.length;
        cntPend = dataPending.length;
    }
    const cntUser = ref(0);
    const cntNV = ref(0);
    ...
    const getPageData = (path) => {
        if (path.includes('pending')) {
            ...
            totalPage = Math.ceil(dataAll?.length / ItemPageLimit) || 0
            page.value = 'all'
            return dataAll
        }
    }
