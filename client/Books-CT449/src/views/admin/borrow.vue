    ...
    <div v-if="!loading" class="relative w-full h-[100vh] flex items-start justify-center gap-5 p-2 font-text1"
        ...
        <div
            ...
            <div class="h-full overflow-hidden">
                <div v-if="!visibleTask.length>0 && !loading"
                    class="h-[85%] mb-5 flex flex-col items-center justify-center gap-5">
                    <img src="../../../public/notfound.png" alt="NotFound" class="h-[80%]">
                    <h3 class="text-2xl uppercase text-gray-400">Chưa có dữ liệu</h3>
                </div>
                <div v-if="!loading && visibleTask.length>0"
                    class="flex flex-col gap-[5%] h-[85%] mb-5 overflow-hidden">
                    <BorrowItem v-if="page=='pending' || page==='all'" v-for="(item, index) in visibleTask"
                        :id="item._id" :key="index" :prop1="item.userid.ten" :prop2="item.bookid.TENSACH"
                        :prop3="item.ngaymuon" :bookid="item.bookid._id" :status="item.status" :page="page"
                    ...
                    <BorrowItem v-if="page==='nhanvien'" v-for="(item, index) in visibleTask" :key="index"
                        :id="item._id" :prop1="item.nameNV" :prop2="item.addressNV" :prop3="item.phoneNV" :page="page"
                    ...
                    <BorrowItem v-if="page==='nxb'" v-for="(item, index) in visibleTask" :key="index" :id="item._id"
                        :prop1="item.MANXB" :prop2="item.TENNXB" :prop3="item.DIACHI" :page="page"
                        @details="handleDetails" @delete="handleDelete" :darkmode="darkmode">
                    </BorrowItem>
                    ...
                    <BorrowItem v-if="page==='author'" v-for="(item, index) in visibleTask" :key="index" :id="item._id"
                        :prop1="item.MATACGIA" :prop2="item.TENTACGIA" :page="page" @details="handleDetails"
                        @delete="handleDelete" :darkmode="darkmode">
                    </BorrowItem>
                </div>
                <div class="h-[10%] flex items-center justify-between">
                    ...
                    <div class="flex gap-5">
                        <Button v-if="page==='all'" goto="/addborrow" title="Tạo đơn" icon="fa-plus"
        ...
        <a-modal v-model:open="openModalComplete" title="Xác nhận" :confirm-loading="confirmLoading"
            @ok="handleOkComplete">
