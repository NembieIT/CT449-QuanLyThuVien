    ...
    <div class="relative w-full h-[100vh] flex items-start justify-center gap-5 p-2 font-text1"
        ...
        <div class="w-[95%] lg:w-[80vw] h-full flex flex-col gap-5 rounded-2xl p-5 bg-white">
            ...
            <div class="flex flex-col gap-[5%] h-[55%] overflow-hidden">
                <BorrowItem v-if="page=='pending' || page==='all'" v-for="(item, index) in visibleTask" :key="index"
                    :name="item.name" :email="item.email" :phone="item.sdt" :status="page=='user'? '': item.status"
                    :page="page">
                </BorrowItem>
                <BorrowItem v-if="page==='user'" v-for="(item, index) in visibleTask" :key="index" :id="item._id"
                    :name="item.ten" :email="item.address" :phone="item.phone" :status="page=='user'? '': item.status"
                    :page="page" @details="handleDetails" @deleteUser="handleDeleteUser">
                </BorrowItem>
        ...
        <a-drawer width="400" placement="right" :closable="false" :open="open" @close="onClose" class="font-text1">
            <div class="h-[60%]">
                ...
                <p class="text-2xl font-text1 font-bold">Họ lót: <span
                        class="text-red-500">{{currentUserDetail.holot}}</span>
                </p>
                <p class="text-2xl font-text1 font-bold">Tên : <span
                        class="text-red-500">{{currentUserDetail.ten}}</span>
                </p>
                ...
                <p class="text-2xl font-text1 font-bold">Giới tính: <span
                        class="text-red-500">{{currentUserDetail.sex}}</span>
                </p>
            ...
            <div class="absolute bottom-[40%] left-5">
                <Button goto="/adduser" title="Chỉnh sửa" icon="fa-plus" class="text-2xl"></Button>
            </div>
        </a-drawer>

        <a-modal v-model:open="openModal" title="Xác nhận" :confirm-loading="confirmLoading" @ok="handleOk">
