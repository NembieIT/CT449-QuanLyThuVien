<template lang="">
    <div v-if="loaded" class="h-full w-full bg-gray p-5">
        <div class="h-[10%] flex items-center justify-between p-5 bg-white">
            <router-link to="/all/hot"
                :class="[StateBook?'bg-blue-400':'bg-gray-500',`text-2xl p-3 rounded-2xl hover:bg-gray-300 transition-all`]">Tất
                cả sách
                hot</router-link>
            <router-link to="/all/newest"
                :class="[StateBook?'bg-gray-500':'bg-blue-400',`text-2xl p-3 rounded-2xl hover:bg-gray-300 transition-all`]">Sách
                mới
                nhất</router-link>
        </div>
...
<script setup>
    ...
    watch(() => props.searchValue, () => {
        ...
        if (!searchFinal) {
            if (StateBook.value) {
                ...
                visibleTask.value = bookHot.value;
            }
        } else {
            if (StateBook.value) {
                ...
                visibleTask.value = bookHot.value.filter(item => {
                    const tensach = removeVietnameseTones(item.TENSACH);
                    if (tensach.includes(searchFinal)) return true
                })
            }
    ...
    onMounted(async () => {
        bookHot.value = props.dataBook.filter(item => item.FAV > 0);
        if (route.fullPath.includes('hot')) {
            visibleTask.value = bookHot.value;
            StateBook.value = false;
        } else {
            visibleTask.value = props.dataBook;
            StateBook.value = true;
        }
        loaded.value = true;
    })
</script>