<template>
    ...
    <div v-else class="h-full w-full p-5 flex flex-col gap-[6%]">
        <div class="h-fit w-full bg-white p-5">
            <div class="h-[10%] flex items-center justify-between">
                ...
                <div
                    ...
                    <router-link to="/all/hot" class="uppercase">See All</router-link>
                    <RightOutlined />
                </div>
            </div>
            <div class="h-[80%] flex items-center justify-center gap-[5%] p-5 flex-wrap overflow-scroll mt-5">
                <div v-if="bookHot.length > 0" v-for="(item, index) in bookHot" :key="item._id || index"
                    class="w-[40%] md:w-[35%] 2xl:w-[20%] h-1/2 2xl:h-full bg-gray-500/40 rounded-2xl overflow-hidden cursor-pointer flex flex-col justify-between">
        ...
        <div class="h-fit w-full bg-white p-5">
            <div class="h-[10%] flex items-center justify-between">
                ...
                <div
                    class="flex items-center text-blue-500 cursor-pointer hover:bg-blue-500 hover:text-white p-2 rounded-2xl transition-all">
                    <router-link to="/all/newest" class="uppercase">See All</router-link>
                    <RightOutlined />
                </div>
            </div>
            <div class="h-[80%] flex items-center justify-center gap-[5%] p-5 flex-wrap overflow-scroll mt-5">
                <div v-if="newestBook.length > 0" v-for="(item, index) in newestBook" :key="item._id || index"
                    class="w-[40%] md:w-[35%] 2xl:w-[20%] h-1/2 2xl:h-full bg-gray-500/40 rounded-2xl overflow-hidden cursor-pointer flex flex-col justify-between">
                    <img class="h-1/2 xl:h-[60%] w-full object-cover hover:object-contain hover:scale-90 transition-all rounded-2xl"
                        @click="handleDetail(item)"
                        :src="item.IMAGEURL || 'https://play.google.com/books/publisher/content/images/frontcover/j5jzEAAAQBAJ?fife=w256-h256'"
